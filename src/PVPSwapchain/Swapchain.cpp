...
static VkSurfaceFormatKHR get_best_surface_format(
...
static VkPresentModeKHR get_best_present_mode(
const VkPhysicalDevice& physical_device,
const VkSurfaceKHR&     surface)
{
...
static uint32_t
get_mini_image_count(const VkSurfaceCapabilitiesKHR& capabilities)
{
...
static VkExtent2D get_swap_chain_extent(
    ...
    : m_instance(&instance)
{
    ...
    m_swapchain_surface_format =
    get_best_surface_format(physical_device, surface);
    create_the_swapchain(device, command_buffer);
}

bool pvp::Swapchain::does_device_support_swapchain(
...
void pvp::Swapchain::create_frame_buffers(VkDevice     device,
                                          VkRenderPass render_pass)
{
    ...
    {
        std::vector             attachments = { m_swapchain_image_views[i],
                                                m_depth_buffer_image->get_view() };
        ...
        if (vkCreateFramebuffer(
            device, &framebuffer_info, nullptr, &m_framebuffers[i]) !=
            VK_SUCCESS)
        {
...
CommandBuffer& command_buffer)
{
    ...
    if (queue_families.graphics_family.family_index !=
        queue_families.present_family.family_index)
    {
    ...
    device.get_physical_device(), m_instance->get_surface());
    create_info.clipped = VK_TRUE;
    create_info.oldSwapchain = VK_NULL_HANDLE;

    if (vkCreateSwapchainKHR(
        device.get_device(), &create_info, nullptr, &m_swapchain) !=
        VK_SUCCESS)
    {
    ...
    for (size_t i = 0; i < m_swapchain_images.size(); ++i)
    {
        ...
        if (vkCreateImageView(device.get_device(),
                              &view_info,
                              nullptr,
        ...
        m_swap_chain_destructor.add_to_queue(
        [&, view_ptr = m_swapchain_image_views[i]] {
